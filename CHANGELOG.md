...
- **The source you pick is the source that plays.** The player used to treat a 30-second start timeout the same for every launch: it auto-hopped to the next-ranked source, so a specific 4K, HDR, or 1080p source you chose could silently drop to a cached 480p while it was still starting, and a big 4K remux on slow debrid could be given up on mid-fill. Now an explicit pick is honored: the player retries that exact source in place with extra first-buffer grace (and extends further while bytes are still arriving), and only surfaces a clear "choose another source" message if it genuinely fails, instead of substituting a different, lower-quality one. Watch Now and Continue Watching still auto-hop off a dead source, but the automatic hop is capped so it never lands more than one resolution tier below a cached higher-quality option, preferring cached sources first. The "tried several sources" overlay no longer flashes while a hop or a resolve is still in flight. Mirrored on iPhone, iPad, Mac, and Apple TV.
